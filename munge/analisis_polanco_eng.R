...
cuenta_diaria %>%
  ...
  mutate(dia_semana_retiro=factor(dia_semana_retiro, levels=dias), horas_retiro=ifelse(horas_retiro==0,24,horas_retiro),
         horas_retiro=hm(paste0(horas_retiro,":00")))%>%
  # filter(horas_retiro>5 ) %>%
         # , horas_retiro=paste0(if(horas_retiro<10){"0"}, horas_retiro,":00")) %>%
  ggplot( aes(x= horas_retiro, y= viajes, group=dia_semana_retiro)) +
# ggplot( aes(x= horas_retiro, y= viajes, group=esp_enum)) +
  geom_line() +
  labs(x="Time of Retrieval", y="Total Trips") +
